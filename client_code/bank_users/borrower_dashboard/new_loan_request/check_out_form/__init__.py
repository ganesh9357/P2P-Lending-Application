...
class check_out_form(check_out_formTemplate):
  def __init__(self, **properties):
        ...
        user_request = app_tables.product_categories.get(name=self.product_categories)

        if user_request:
            self.interest_rate = user_request['roi']
            self.int_rate.text = f"Interest rate : {self.roi} %"
        else:
