...
#         membership_type = getattr(self.drop_down_2, 'selected_value', None)
...
def button_1_click(self, **event_args):
    ...
    # Check if drop_down_2 has a selected value
    membership_type = getattr(self.drop_down_2, 'selected_value', None)
    print(f"Membership Type: {membership_type}")

    interest_type = self.radio_button_1.text if self.radio_button_1.checked else self.radio_button_2.text
    ...
    # Check if foreclose_type has a selected value
    foreclose_type = getattr(self.foreclose_type, 'selected_value', None)
    print(f"Foreclose Type: {foreclose_type}")
    ...
    # Check if extension_allowed has a selected value
    extension_allowed = extension_allowed_mapping.get(getattr(self.extension_allowed, 'selected_value', None), False)
    print(f"Extension Allowed: {extension_allowed}")

    emi_payment = getattr(self.drop_down_1, 'selected_value', None)
    print(f"EMI Payment: {emi_payment}")

    # Check if radio_button_3 has a selected value
    discount_coupons = self.radio_button_3.text if self.radio_button_3.checked else self.radio_button_4.text
    print(f"Discount Coupons: {discount_coupons}")
